...
#' @param harmonic_eigenvalues function of two variables, \code{omega} and \code{n}, that assigns the \code{n}-th harmonic eigenvalue at frequency \code{omega}
#' @param harmonic_eigenfunctions function of three variables, \code{omega}, \code{n} and \code{x}, that assigns the \code{n}-th harmonic eigenfunction at point \code{x} in [0,1] at frequency \code{omega}
#' @param t_max Time horizon to be sumilated. Must be an even number.
#' @param n_grid Number of grid points (spatial resolution) of the discretisation of [0,1] where the FTS is to be simulated
#' @param n_pc 
#' @param seed_number
#' @param include_freq_zero 
#' @return functional time series sample, matrix of size (n_grid,t_max)
#' @references Rubin, Panaretos.
#' \emph{Simulation of stationary functional time series with given spectral density}.
#' arXiv, 2020
#' @seealso \code{\link{spec_density_simulate}}
#' @examples
